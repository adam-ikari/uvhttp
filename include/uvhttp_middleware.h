...
#define UVHTTP_EXECUTE_MIDDLEWARE(req, resp, ...) \
    do { \
        ...
        for (size_t _uvhttp_mw_i = 0; _uvhttp_mw_i < sizeof(_uvhttp_mw_handlers)/sizeof(_uvhttp_mw_handlers[0]); _uvhttp_mw_i++) { \
            if (_uvhttp_mw_handlers[_uvhttp_mw_i] && _uvhttp_mw_handlers[_uvhttp_mw_i](req, resp, &_uvhttp_mw_ctx) != UVHTTP_MIDDLEWARE_CONTINUE) { \
                if (_uvhttp_mw_ctx.cleanup) _uvhttp_mw_ctx.cleanup(_uvhttp_mw_ctx.data); \
                goto _uvhttp_mw_stop; \
            } \
        } \
    } while(0); \
    _uvhttp_mw_stop: (void)0
...
#define UVHTTP_EXECUTE_MIDDLEWARE_CHAIN(req, resp, name) \
    do { \
        ...
        for (size_t _uvhttp_mw_i = 0; _uvhttp_mw_i < name##_count; _uvhttp_mw_i++) { \
            if (name##_handlers[_uvhttp_mw_i] && name##_handlers[_uvhttp_mw_i](req, resp, &_uvhttp_mw_ctx) != UVHTTP_MIDDLEWARE_CONTINUE) { \
                if (_uvhttp_mw_ctx.cleanup) _uvhttp_mw_ctx.cleanup(_uvhttp_mw_ctx.data); \
