...
void test_mtls_config_setup() {
    ...
    TEST_ASSERT(strcmp(config.server_cert_file, "certs/server.crt") == 0, "服务器证书文件设置正确");
    TEST_ASSERT(strcmp(config.server_key_file, "certs/server.key") == 0, "服务器私钥文件设置正确");
    TEST_ASSERT(strcmp(config.ca_cert_file, "certs/ca.crt") == 0, "CA证书文件设置正确");
    TEST_ASSERT(strcmp(config.client_cert_file, "certs/client.crt") == 0, "客户端证书文件设置正确");
    TEST_ASSERT(strcmp(config.client_key_file, "certs/client.key") == 0, "客户端私钥文件设置正确");
