...
static int run_stress_test(stress_test_config_t* config) {
    ...
    // 初始化测试超时定时器
    uv_timer_init(g_test_manager.loop, &g_test_manager.timeout_timer);
    g_test_manager.timeout_timer.data = &g_test_manager;
    uv_timer_start(&g_test_manager.timeout_timer, on_test_timeout, config->test_duration_seconds * 1000, 0);
