...
// 测试极端输入
TEST(UtilsTest, ExtremeInputs) {
    char dest[10];
    
    // 测试包含特殊字符的字符串
    EXPECT_EQ(safe_strncpy(dest, "test\t\n\r", sizeof(dest)), 0);
    EXPECT_EQ(safe_strncpy(dest, "test\x01\x02", sizeof(dest)), 0);
    
    // 测试空字符串和单字符
    EXPECT_EQ(safe_strncpy(dest, "", sizeof(dest)), 0);
    EXPECT_EQ(safe_strncpy(dest, "x", sizeof(dest)), 0);
    EXPECT_EQ(dest[0], 'x');


// 测试内存安全
TEST(UtilsTest, MemorySafety) {
    char dest[10];
    const char* test_str = "test";
    
    // 测试多次操作不会导致内存问题
    for (int i = 0; i < 100; i++) {
        EXPECT_EQ(safe_strncpy(dest, test_str, sizeof(dest)), 0);
        EXPECT_STREQ(dest, test_str);
    }
    
    // 测试边界情况不会越界
    EXPECT_EQ(safe_strncpy(dest, "123456789012345", sizeof(dest)), 0);
    EXPECT_TRUE(strlen(dest) < sizeof(dest));


// 测试输入验证的严格性
TEST(UtilsTest, InputValidationStrictness) {
    // 测试URL验证的严格性
    EXPECT_EQ(validate_url("/valid/path", 11), 0);
    EXPECT_EQ(validate_url("/path\x00withnull", 16), -1);
    EXPECT_EQ(validate_url("", 0), -1);
    
    // 测试头部值验证的严格性
    EXPECT_EQ(validate_header_value("valid-value", 11), 0);
    EXPECT_EQ(validate_header_value("invalid\x00value", 14), -1);
    EXPECT_EQ(validate_header_value("", 0), -1);
    
    // 测试方法验证的严格性
    EXPECT_EQ(validate_method("GET", 3), 0);
    EXPECT_EQ(validate_method("get", 3), -1); // 大小写敏感
    EXPECT_EQ(validate_method("INVALID", 7), -1);
    EXPECT_EQ(validate_method("", 0), -1);
