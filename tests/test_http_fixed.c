...
TEST(HttpTest, ResponseStatusCode) {
    ...
    // 测试边界状态码
    uvhttp_response_set_status(&response, 100);
    EXPECT_EQ(response.status_code, 100);
    
    uvhttp_response_set_status(&response, 599);
    EXPECT_EQ(response.status_code, 599);
...
TEST(HttpTest, ResponseHeaders) {
    ...
    // 测试空头部值
    uvhttp_response_set_header(&response, "name", "");
...
TEST(HttpTest, ResponseBody) {
    ...
    // 测试零长度响应体
    EXPECT_EQ(uvhttp_response_set_body(&response, body, 0), 0);
    EXPECT_EQ(response.body_length, 0);
...
TEST(HttpTest, HttpMethodParsing) {
    ...
    for (int i = 0; i < method_count; i++) {
        ...
        // 测试方法验证
        EXPECT_EQ(validate_method(methods[i], strlen(methods[i])), 0);
    }
...
TEST(HttpTest, HttpHeaderParsing) {
    ...
    EXPECT_EQ(validate_header_value(value, strlen(value)), 0);
...
TEST(HttpTest, UrlParsing) {
    ...
    const char* urls[] = {
        ...
        "/users/123/profile",
        "/api/v1/users/1234567890123456789012345678901234567890"
    };
    ...
    // 测试无效URL
    EXPECT_EQ(validate_url("relative/path", 14), -1); // 相对路径
    EXPECT_EQ(validate_url("", 0), -1); // 空URL
...
TEST(HttpTest, HttpVersionParsing) {
    ...
    // 测试无效版本格式
    const char* invalid_version = "HTTP/2.0";
    EXPECT_EQ(strncmp(invalid_version, "HTTP/", 5), 0);
    EXPECT_NE(strncmp(invalid_version + 5, "2.0", 3), 0); // 应该不匹配1.1或1.0
}
...
// 测试边界条件
TEST(HttpTest, BoundaryConditions) {
    uvhttp_response_t response;
    memset(&response, 0, sizeof(response));
    uvhttp_response_init(&response, NULL);
    ...
    EXPECT_EQ(response.status_code, 999);
    
    // 测试空头部
    uvhttp_response_set_header(&response, "", "");
