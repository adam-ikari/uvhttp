/**
 * @file helloworld_complete.c
 * @brief UVHTTP Hello World (complete) - a more thorough setup showing config
 *        validation, application context, request counting, and runtime config
 *        reporting in the response.
 *
 * Demonstrates the full server bring-up path with the output-parameter style
 * constructors: loop -> config (validated) -> context -> server -> router.
 */
...
/* File-scope pointer so handlers can reach the app context. */
...
static void app_context_free(app_context_t* ctx) {
    if (ctx) {
        ...
        /* Router is freed by uvhttp_server_free. */
        if (ctx->config) {
...
int main() {
    ...
    /* Track whether we still own the config for error-path cleanup. */
    int cleanup_needed = 1;

    /* Set values directly (no defaults applied first). */
    ...
    /* Validation succeeded: the context/server now takes ownership. */
    cleanup_needed = 0;
