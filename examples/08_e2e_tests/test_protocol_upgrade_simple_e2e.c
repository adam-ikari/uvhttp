...
static int http_handler(uvhttp_request_t* request,
...
static int websocket_upgrade_handler(uvhttp_request_t* request,
                                     uvhttp_response_t* response) {
    ...
    if (upgrade_header && strstr(upgrade_header, "websocket")) {
        ...
        const char* body =
            "WebSocket upgrade requested (101 Switching Protocols)";
        ...
        uvhttp_response_set_header(response, "Sec-WebSocket-Accept",
                                   "simulated");
        uvhttp_response_set_header(response, "Content-Type", "text/plain");
        uvhttp_response_set_body(response, body, strlen(body));
        uvhttp_response_send(response);
...
static int send_http_request(const char* method, const char* path,
    ...
    snprintf(cmd, sizeof(cmd),
             "curl -s -X %s http://127.0.0.1:8778%s 2>/dev/null", method, path);
...
static int send_http_request_with_headers(const char* method, const char* path,
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd),
             "curl -s -X %s %s http://127.0.0.1:8778%s 2>/dev/null", method,
             headers, path);
...
int main(int argc, char** argv) {
    ...
    uvhttp_router_add_route(app.router, "/upgrade-check",
                            upgrade_check_handler);
    uvhttp_router_add_route(app.router, "/websocket",
    ...
    send_http_request_with_headers(
        "GET", "/upgrade-check",
        "-H 'Upgrade: websocket' -H 'Connection: Upgrade'", response,
        sizeof(response));
    ...
    send_http_request_with_headers("GET", "/websocket",
                                   "-H 'Upgrade: websocket' -H 'Connection: "
                                   "Upgrade' -H 'Sec-WebSocket-Key: test'",
