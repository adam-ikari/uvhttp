...
static int string_ops_handler(uvhttp_request_t* request,
...
static int number_ops_handler(uvhttp_request_t* request,
                              uvhttp_response_t* response) {
    ...
    snprintf(response_body, sizeof(response_body),
             ...
             "Is Positive: %s\n"
             "Is Even: %s",
             num_str, num, (num < 0) ? -num : num, (num > 0) ? "Yes" : "No",
             (num % 2 == 0) ? "Yes" : "No");
...
static int encoding_handler(uvhttp_request_t* request,
                            uvhttp_response_t* response) {
    const char* input = uvhttp_request_get_header(request, "X-Input");
    ...
    snprintf(response_body, sizeof(response_body),
             ...
             "First char: %c\n"
             "Last char: %c",
             input, strlen(input), (strchr(input, ' ') != NULL) ? "Yes" : "No",
...
static int timestamp_handler(uvhttp_request_t* request,
                             uvhttp_response_t* response) {
    ...
    snprintf(response_body, sizeof(response_body),
             ...
             "Month: %d\n"
             "Day: %d",
             (long)now, time_str, tm_info->tm_year + 1900, tm_info->tm_mon + 1,
             tm_info->tm_mday);
...
static int send_http_request(const char* method, const char* path,
    ...
    snprintf(cmd, sizeof(cmd),
             "curl -s -X %s http://127.0.0.1:8781%s 2>/dev/null", method, path);
...
static int send_http_request_with_header(const char* method, const char* path,
    ...
    char cmd[1024];
    snprintf(cmd, sizeof(cmd),
             "curl -s -X %s -H '%s' http://127.0.0.1:8781%s 2>/dev/null",
             method, header, path);
...
int main(int argc, char** argv) {
    ...
    send_http_request_with_header("GET", "/utils/string",
                                  "X-Input: hello world", response,
                                  sizeof(response));
    ...
    send_http_request_with_header("GET", "/utils/number", "X-Number: -42",
    ...
    send_http_request_with_header("GET", "/utils/encoding",
                                  "X-Input: test@example.com", response,
                                  sizeof(response));
