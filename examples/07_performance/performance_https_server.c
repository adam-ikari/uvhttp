...
int main(int argc, char* argv[]) {
    ...
    int threads = 1;
    (void)threads;  /* Single event loop; -t is accepted for CLI compatibility */
    ...
    /* Create server */
    uvhttp_error_t err = uvhttp_server_new(ctx.loop, &ctx.server);
    if (err != UVHTTP_OK || !ctx.server) {
    ...
    if (err != UVHTTP_OK || !router) {
        fprintf(stderr, "Error: Failed to create router: %d\n", err);
    ...
    err = uvhttp_tls_context_load_cert_chain(tls_ctx, cert_file);
    if (err != UVHTTP_OK) {
    ...
    /* Load private key */
    err = uvhttp_tls_context_load_private_key(tls_ctx, key_file);
    if (err != UVHTTP_OK) {
    ...
    /* Configure server with TLS. On success the server owns tls_ctx and
     * releases it in uvhttp_server_free; do not free it ourselves. */
    err = uvhttp_server_enable_tls(ctx.server, tls_ctx);
    if (err != UVHTTP_OK) {
