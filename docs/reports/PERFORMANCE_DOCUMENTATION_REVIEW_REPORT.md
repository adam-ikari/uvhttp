...
1. **基准性能数据不准确**:
   - 文档基准: 小文件 35,000 QPS
   - 实际测试: 约 8,800-10,000 QPS
   - **差异**: 约 65-75% 的性能差距

2. **优化效果数据不真实**:
   - 文档声称小文件优化后达到 34,479 QPS
   - 实际测试无法达到此性能
...
| 测试场景 | 线程数 | 连接数 | 实际 QPS | 文档基准 QPS | 差异 | 平均延迟 | 传输速率 |
|---------|-------|--------|---------|------------|------|---------|---------|
...
| 文件类型 | 文件大小 | 实际 QPS | 文档基准 QPS | 差异 | 平均延迟 | 传输速率 |
|---------|---------|---------|------------|------|---------|---------|
...
| 测试场景 | 实际 QPS | 文档基准 QPS | 性能差距 | 可能原因 |
|---------|---------|------------|---------|---------|
...
- **低并发**: 12,990 QPS（文档基准: 35,864 QPS，差距: -63.8%）
- **中等并发**: 10,385 QPS（文档基准: 16,544 QPS，差距: -37.2%）
- **高并发**: 10,137 QPS（文档基准: 14,015 QPS，差距: -27.7%）
