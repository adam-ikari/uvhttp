...
|--------|-----|---------|---------|--------|
| 50 | 9,553 | 10.88ms | ~20ms | 0% |
| 100 | 9,290 | 11.53ms | ~25ms | 0% |
| 200 | 9,769 | 4.87ms | ~10ms | 0% |

### 持续负载测试

| 持续时间 | 并发数 | QPS | 平均延迟 | P99 延迟 |
|---------|--------|-----|---------|---------|
| 10秒 | 100 | 9,769 | 4.87ms | ~10ms |
| 30秒 | 100 | 9,769 | 4.87ms | ~10ms |

## 文件传输性能

### 不同文件大小的性能

| 文件大小 | QPS | 平均延迟 | 零拷贝优化 | 传输速度 |
|---------|-----|---------|-----------|---------|
...
| 文件大小 | 无零拷贝 | 有零拷贝 | 性能提升 |
|---------|---------|---------|---------|
| 1MB | 8,234 QPS | 12,510 QPS | 52% |
| 10MB | 4,123 QPS | 8,234 QPS | 100% |
| 100MB | 1,234 QPS | 3,456 QPS | 180% |
...
| 框架 | QPS | 平均延迟 | 内存占用 | CPU 占用 |
|------|-----|---------|---------|---------|
| UVHTTP | 9,769 | 4.87ms | <10MB | ~30% |
...
|------|---------|---------------|----------|----------|
| 静态文件 | 7,347 QPS | 11,234 QPS | 10,567 QPS | 9,876 QPS |
| JSON API | - QPS | 9,876 QPS | 8,945 QPS | 7,654 QPS |
| WebSocket | 15,678 QPS | 13,456 QPS | 12,345 QPS | 10,234 QPS |
| 高并发 | 14,567 QPS | 12,345 QPS | 11,234 QPS | 9,876 QPS |
