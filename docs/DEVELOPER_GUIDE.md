...
UVHTTP 采用 **Linux 内核开发节奏** 的分支管理策略：持续开发 - 快速修复 - 稳定发布。
...
- **主版本 (X.Y.0)**: 新功能发布，从 `develop` 合并
- **补丁版本 (X.Y.Z)**: Bug 修复，从 `main` 创建 `fix/` 分支
- **修复同步**: `main` 的修复必须同步到 `develop`
