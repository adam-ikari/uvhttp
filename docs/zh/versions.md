...
- 测试改进：93 项测试（之前 91），Connection 覆盖率 42.8% → ~70%
- WebSocket 自动化集成测试：99 个测试用例
- Router 覆盖测试：find_array_route 和 match_route_node
- SDD 规格文档：17 个（之前 14 个）
- ASan/UBSan 93/93 通过
- uvhttp_server_new_with_loop 内部事件循环管理 API
- Daily-build CI 工作流
- 开发节奏文档
- 各种文档修复和一致性改进
