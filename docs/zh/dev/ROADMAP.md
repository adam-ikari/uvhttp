...
以下功能不符合 uvhttp 的设计哲学，不会包含在路线图中：
...
- ❌ 实时性能仪表板、APM 集成（独立工具）
- ❌ 性能预测（需要机器学习）
- ❌ API 版本管理、API 文档自动生成（应用层）
- ❌ Mock 服务器、请求/响应录制（测试工具）

### 核心设计原则

1. **专注核心功能**：只处理 HTTP/1.1 和 WebSocket
2. **轻量级**：393KB 静态库，低资源占用
3. **可嵌入性**：可嵌入到任何 C/C++ 应用
4. **可定制化**：中间件系统，编译时配置
5. **模块化**：通过编译宏控制功能
6. **职责清晰**：应用层负责业务逻辑、JSON 解析、数据库集成
