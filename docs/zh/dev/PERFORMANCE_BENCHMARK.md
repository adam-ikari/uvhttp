...
#### 不同响应类型性能（4 线程 / 100 连接）

| 响应类型 | RPS | 平均延迟 | 传输速率 |
|---------|-----|---------|---------|
| 简单文本 (13B) | **21,991** | 4.56 ms | 2.43 MB/s |
| JSON 响应 (50B) | **21,095** | 4.74 ms | 3.20 MB/s |
| 小响应 (1KB) | **21,395** | 4.67 ms | 23.02 MB/s |

### 峰值性能总结

- **峰值 RPS**: **21,991** (高并发场景，简单文本)
- **最佳延迟**: **551 μs** (低并发场景)
- **最高吞吐**: **23.02 MB/s** (小响应场景)
...
| 测试日期 | 峰值 RPS | 主要优化 |
|---------|---------|---------|
| 2026-01-30 | 20,432 | 路由缓存优化 |
| 2026-02-02 | 21,991 | 路由缓存 + bug 修复 |
| **提升** | **+7.6%** | 参数化路由修复 + 递归深度限制 |
