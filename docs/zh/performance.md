...
> 吞吐量随硬件而异。以下为近期在 Linux 虚拟机（Linux 6.17，x86_64）上使用
> `wrk 4.1.0` 对内置 `test_performance_e2e` 服务器（Release 构建）的测量结果。
> 复现命令：`wrk -t4 -c100 -d10s http://127.0.0.1:18080/simple`。原始基准主机上
> 曾测得 100 连接下最高约 31K RPS。
...
| 测试场景 | RPS | 平均延迟 | 备注 |
|---------|-----|---------|---------|
| 低并发（10 连接） | ~20K | ~0.4ms | 峰值 |
| 高并发（4 线程 / 100 连接） | ~17–19K | ~9ms（P50） | 零错误 |
| 超高并发（1000 连接） | ~19K | — | 零错误 |
| JSON 端点（100 连接） | ~19.7K | ~9–21ms（P50–P90） | |
| 大响应 1KB（100 连接） | ~19.4K | — | 9.85MB/s |

### 性能提升

| 版本 | 峰值 RPS | 提升 | 主要优化 |
|------|---------|------|----------|
| v2.3.0 | **31,883** | +138% | 移除热路径缓存，统一 benchmark 编译选项 |
...
| 测试场景 | RPS | 平均延迟 | 最大延迟 |
|---------|-----|---------|---------|
| 高并发（4 线程 / 100 连接） | 31,883 | 3.09ms | 49.83ms |
...
|---------|-----|---------|---------|--------|
| 低并发（2 线程 / 10 连接） | 24,439 | 362 μs | 3.35 ms | 0% |
| 中等并发（4 线程 / 50 连接） | 23,959 | 1.97 ms | 5.40 ms | 0% |
| 高并发（8 线程 / 200 连接） | 23,273 | 8.57 ms | 22.39 ms | 0% |

### 持续负载测试

| 持续时间 | 测试场景 | RPS | 平均延迟 | P99 延迟 |
|---------|---------|-----|---------|---------|
| 10秒 | 中等并发（4 线程 / 50 连接） | 23,959 | 1.97 ms | 5.40 ms |
| 30秒 | 中等并发（4 线程 / 50 连接） | 23,900 | 2.00 ms | 5.50 ms |
| 60秒 | 中等并发（4 线程 / 50 连接） | 23,850 | 2.05 ms | 5.60 ms |
...
|------|---------|---------------|----------|----------|
| 低并发 | 24,439 RPS | 15,234 RPS | 13,456 RPS | 11,234 RPS |
| 中等并发 | 23,959 RPS | 14,567 RPS | 12,345 RPS | 10,567 RPS |
| 高并发 | 23,273 RPS | 13,890 RPS | 11,234 RPS | 9,876 RPS |
