...
features:
  - title: 🚀 极致性能
    details: 峰值吞吐量达 24,439 RPS，基于 libuv 事件驱动，集成 xxHash 极快哈希算法
  - title: ⚡ 零拷贝优化
...
- **低并发吞吐量**：24,439 RPS（2 线程 / 10 连接）
- **中等并发吞吐量**：23,959 RPS（4 线程 / 50 连接）
- **高并发吞吐量**：23,273 RPS（8 线程 / 200 连接）
- **平均延迟**：362 μs - 8.57 ms
- **P99 延迟**：1.6 ms - 22.39 ms
- **错误率**：< 0.1%
- **并发连接**：支持数千并发连接
