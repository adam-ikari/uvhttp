...
**After:**
```c
// Use priority-based eviction instead
uvhttp_lru_cache_enable_priority_eviction(cache, 1);
```
...
**After:**
```c
// No longer needed - eviction is now synchronous
```
...
**Solution:** Remove calls to removed functions. See [Migration Guide](#step-by-step-migration) for details.

#### Issue 2: Performance Regression

**Symptom:** Cache eviction slower than before

**Solution:** Enable priority-based eviction for better control:
```c
uvhttp_lru_cache_enable_priority_eviction(cache, 1);
uvhttp_lru_cache_set_min_priority_threshold(cache, 5);
```
