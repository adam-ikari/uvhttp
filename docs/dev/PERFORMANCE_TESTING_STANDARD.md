...
| 指标 | 最低要求 | 推荐值 | 实际基准 |
|-----|---------|-------|---------|
| 吞吐量 (RPS) | 10,000 | 15,000 | 24,439 |
| 平均延迟 | < 1 ms | < 600 μs | 362 μs |
| P99 延迟 | < 10 ms | < 5 ms | 3.35 ms |

#### 中等并发场景（4 线程 / 50 连接）
| 指标 | 最低要求 | 推荐值 | 实际基准 |
|-----|---------|-------|---------|
| 吞吐量 (RPS) | 10,000 | 15,000 | 23,959 |
| 平均延迟 | < 10 ms | < 5 ms | 1.97 ms |
| P99 延迟 | < 50 ms | < 20 ms | 5.40 ms |

#### 高并发场景（8 线程 / 200 连接）
| 指标 | 最低要求 | 推荐值 | 实际基准 |
|-----|---------|-------|---------|
| 吞吐量 (RPS) | 10,000 | 15,000 | 23,273 |
| 平均延迟 | < 50 ms | < 20 ms | 8.57 ms |
| P99 延迟 | < 200 ms | < 100 ms | 22.39 ms |
