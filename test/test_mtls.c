...
mock_tls_context_t* mock_tls_context_new() {
    ...
    // 模拟证书路径
    strcpy(ctx->cert_path, "test/certs/server.crt");
    strcpy(ctx->key_path, "test/certs/server.key");
    strcpy(ctx->ca_path, "test/certs/ca.crt");
...
int main() {
    ...
    if (mock_tls_load_cert(tls_ctx, "test/certs/server.crt") != 0) {
    ...
    if (mock_tls_load_key(tls_ctx, "test/certs/server.key") != 0) {
    ...
    if (mock_tls_load_ca(tls_ctx, "test/certs/ca.crt") != 0) {
