...
static void setup_stress_server() {
    ...
    uvhttp_error_t result = uvhttp_server_listen(g_stress_server, STRESS_TEST_HOST, STRESS_TEST_PORT);
...
TEST(StressTest, RapidConnectDisconnect) {
    ...
    for (int i = 0; i < num_connections; i++) {
        ...
        uv_ip4_addr(STRESS_TEST_HOST, STRESS_TEST_PORT, &addr);
