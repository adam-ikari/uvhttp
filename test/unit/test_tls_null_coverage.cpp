...
TEST(UvhttpTlsNullCoverageTest, TlsInit) {
    ...
    uvhttp_error_t err = uvhttp_tls_init(context);
    EXPECT_TRUE(err == UVHTTP_OK || err == UVHTTP_ERROR_TLS_INIT);
...
TEST(UvhttpTlsNullCoverageTest, TlsContextLoadCertChainNull) {
    uvhttp_error_t err = uvhttp_tls_context_load_cert_chain(NULL, NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试加载私钥NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextLoadPrivateKeyNull) {
    uvhttp_error_t err = uvhttp_tls_context_load_private_key(NULL, NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试加载CA文件NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextLoadCaFileNull) {
    uvhttp_error_t err = uvhttp_tls_context_load_ca_file(NULL, NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试启用客户端认证NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextEnableClientAuthNull) {
    uvhttp_error_t err = uvhttp_tls_context_enable_client_auth(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置验证深度NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetVerifyDepthNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_verify_depth(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置密码套件NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetCipherSuitesNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_cipher_suites(NULL, NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试启用会话票据NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextEnableSessionTicketsNull) {
    uvhttp_error_t err = uvhttp_tls_context_enable_session_tickets(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置会话缓存NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetSessionCacheNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_session_cache(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsSetupSslNull) {
    uvhttp_error_t err = uvhttp_tls_setup_ssl(NULL, -1);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试握手NULL */
TEST(UvhttpTlsNullCoverageTest, TlsHandshakeNull) {
    uvhttp_error_t err = uvhttp_tls_handshake(NULL);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsReadNull) {
    char buf[1024];
    uvhttp_error_t err = uvhttp_tls_read(NULL, buf, sizeof(buf));
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsWriteNull) {
    ...
    uvhttp_error_t err = uvhttp_tls_write(NULL, data, 4);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试启用CRL检查NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextEnableCrlCheckingNull) {
    uvhttp_error_t err = uvhttp_tls_context_enable_crl_checking(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsContextEnableTls13Null) {
    uvhttp_error_t err = uvhttp_tls_context_enable_tls13(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置TLS1.3密码套件NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetTls13CipherSuitesNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_tls13_cipher_suites(NULL, NULL);
    /* 可能返回OK（函数可能忽略NULL） */
}

/* 测试启用早期数据NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextEnableEarlyDataNull) {
    uvhttp_error_t err = uvhttp_tls_context_enable_early_data(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置票据密钥NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetTicketKeyNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_ticket_key(NULL, NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试轮换票据密钥NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextRotateTicketKeyNull) {
    uvhttp_error_t err = uvhttp_tls_context_rotate_ticket_key(NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试设置票据生命周期NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextSetTicketLifetimeNull) {
    uvhttp_error_t err = uvhttp_tls_context_set_ticket_lifetime(NULL, 0);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试验证证书链NULL */
TEST(UvhttpTlsNullCoverageTest, TlsVerifyCertChainNull) {
    uvhttp_error_t err = uvhttp_tls_verify_cert_chain(NULL);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试添加额外链证书NULL */
TEST(UvhttpTlsNullCoverageTest, TlsContextAddExtraChainCertNull) {
    uvhttp_error_t err = uvhttp_tls_context_add_extra_chain_cert(NULL, NULL);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsGetCertChainNull) {
    ...
    uvhttp_error_t err = uvhttp_tls_get_cert_chain(NULL, &chain);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsGetStatsNull) {
    ...
    uvhttp_error_t err = uvhttp_tls_get_stats(NULL, &stats);
    EXPECT_NE(err, UVHTTP_OK);
}

/* 测试重置统计信息NULL */
TEST(UvhttpTlsNullCoverageTest, TlsResetStatsNull) {
    uvhttp_error_t err = uvhttp_tls_reset_stats(NULL);
    EXPECT_NE(err, UVHTTP_OK);
}
...
TEST(UvhttpTlsNullCoverageTest, TlsGetConnectionInfoNull) {
    char buf[256];
    uvhttp_error_t err = uvhttp_tls_get_connection_info(NULL, buf, sizeof(buf));
    EXPECT_NE(err, UVHTTP_OK);
}