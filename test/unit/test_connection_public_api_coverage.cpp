...
/* ========== WebSocket Handle Tests ========== */

TEST_F(UvhttpConnectionPublicAPICoverageTest, HandleWebSocketHandshakeNull) {
    const char* ws_key = "test-key";
    uvhttp_error_t result = uvhttp_connection_handle_websocket_handshake(nullptr, ws_key);
    
    EXPECT_NE(result, UVHTTP_OK);
}

TEST_F(UvhttpConnectionPublicAPICoverageTest, HandleWebSocketHandshakeNullKey) {
    uvhttp_connection_t* conn = nullptr;
    uvhttp_connection_new(server, &conn);
    
    ASSERT_NE(conn, nullptr);
    
    uvhttp_error_t result = uvhttp_connection_handle_websocket_handshake(conn, nullptr);
...
TEST_F(UvhttpConnectionPublicAPICoverageTest, HandleWebSocketHandshakeValid) {
    uvhttp_connection_t* conn = nullptr;
    uvhttp_connection_new(server, &conn);
    ...
    const char* ws_key = "test-key";
    uvhttp_error_t result = uvhttp_connection_handle_websocket_handshake(conn, ws_key);
    
    // Result depends on whether WebSocket is enabled
    // We just verify it doesn't crash
    
    uvhttp_connection_free(conn);
}

/* ========== WebSocket Switch Tests ========== */

TEST_F(UvhttpConnectionPublicAPICoverageTest, SwitchToWebSocketNull) {
    // Should not crash
    uvhttp_connection_switch_to_websocket(nullptr);
}

TEST_F(UvhttpConnectionPublicAPICoverageTest, SwitchToWebSocketValid) {
    uvhttp_connection_t* conn = nullptr;
    uvhttp_connection_new(server, &conn);
...
TEST_F(UvhttpConnectionPublicAPICoverageTest, WebSocketCloseNull) {
    // Should not crash
    uvhttp_connection_websocket_close(nullptr);
}

TEST_F(UvhttpConnectionPublicAPICoverageTest, WebSocketCloseValid) {
    uvhttp_connection_t* conn = nullptr;
    uvhttp_connection_new(server, &conn);
