...
TEST(UvhttpServerSimpleHandlersTest, QuickResponse) {
    ...
    /* 调用快速响应 */
    uvhttp_quick_response(&response, 200, "text/plain", "OK");
...
/* 测试快速响应 NULL 响应 */
TEST(UvhttpServerSimpleHandlersTest, QuickResponseNullResponse) {
    uvhttp_quick_response(NULL, 200, "text/plain", "OK");
    /* 不应该崩溃 */
}

/* 测试 HTML 响应 */
TEST(UvhttpServerSimpleHandlersTest, HtmlResponse) {
    uvhttp_request_t request;
    uvhttp_response_t response;
    memset(&request, 0, sizeof(request));
    memset(&response, 0, sizeof(response));
    ...
    /* 调用 HTML 响应 */
    uvhttp_html_response(&response, "<html><body>Hello</body></html>");
...
/* 测试 HTML 响应 NULL 响应 */
TEST(UvhttpServerSimpleHandlersTest, HtmlResponseNullResponse) {
    uvhttp_html_response(NULL, "<html><body>Hello</body></html>");
    /* 不应该崩溃 */
}

/* 测试 HTML 响应 NULL 内容 */
TEST(UvhttpServerSimpleHandlersTest, HtmlResponseNullContent) {
    uvhttp_response_t response;
    memset(&response, 0, sizeof(response));
    
    uvhttp_html_response(&response, NULL);
    /* 不应该崩溃 */
}

/* 测试文件响应 */
TEST(UvhttpServerSimpleHandlersTest, FileResponse) {
    uvhttp_request_t request;
    uvhttp_response_t response;
    memset(&request, 0, sizeof(request));
    memset(&response, 0, sizeof(response));
    ...
    /* 创建临时文件 */
    const char* test_file = "/tmp/uvhttp_test_file_response.txt";
    ...
    if (fd >= 0) {
        ...
        /* 调用文件响应 */
        uvhttp_file_response(&response, test_file);
...
/* 测试文件响应 NULL 响应 */
TEST(UvhttpServerSimpleHandlersTest, FileResponseNullResponse) {
    uvhttp_file_response(NULL, "/tmp/test.txt");
    /* 不应该崩溃 */
}

/* 测试文件响应 NULL 文件 */
TEST(UvhttpServerSimpleHandlersTest, FileResponseNullFile) {
    uvhttp_response_t response;
    memset(&response, 0, sizeof(response));
    
    uvhttp_file_response(&response, NULL);
    /* 不应该崩溃 */
}
