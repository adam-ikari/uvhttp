...
print_config() {
    ...
    echo ""
    echo "性能基准值（中等并发场景）:"
    echo "  主页 QPS: ${PERFORMANCE_BASELINES[homepage_rps_target]} (最小: ${PERFORMANCE_BASELINES[homepage_rps_min]})"
    echo "  小文件 QPS: ${PERFORMANCE_BASELINES[small_file_rps_target]} (最小: ${PERFORMANCE_BASELINES[small_file_rps_min]})"
    echo "  中等文件 QPS: ${PERFORMANCE_BASELINES[medium_file_rps_target]} (最小: ${PERFORMANCE_BASELINES[medium_file_rps_min]})"
    echo "  大文件 QPS: ${PERFORMANCE_BASELINES[large_file_rps_target]} (最小: ${PERFORMANCE_BASELINES[large_file_rps_min]})"
    echo ""
    echo "性能回归阈值:"
    echo "  QPS 警告: ${RPS_REGRESSION_WARNING}%, 失败: ${RPS_REGRESSION_FAILURE}%"
