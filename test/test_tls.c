...
TEST(TlsTest, CertificateLoading) {
    ...
    // 测试加载服务器证书
    int result = uvhttp_tls_context_load_cert_chain(ctx, "test/certs/server.crt");
    // 由于证书文件可能不存在，这里只测试函数调用不会崩溃
    EXPECT_TRUE(result == 0 || result == -1);
    
    // 测试加载私钥
    result = uvhttp_tls_context_load_private_key(ctx, "test/certs/server.key");
    EXPECT_TRUE(result == 0 || result == -1);
    
    // 测试加载CA证书
    result = uvhttp_tls_context_load_ca_file(ctx, "test/certs/ca.crt");
    EXPECT_TRUE(result == 0 || result == -1);
...
int main() {
    printf("=== Running TLS Tests ===\n");
    
    // 运行所有测试
    TEST_TlsTest_TlsContextCreation();
    TEST_TlsTest_TlsContextFree();
    TEST_TlsTest_CertificateLoading();
    TEST_TlsTest_ClientAuthConfiguration();
    TEST_TlsTest_TlsSecurityConfiguration();
    TEST_TlsTest_CertificateVerification();
    TEST_TlsTest_ErrorHandling();
    TEST_TlsTest_MemoryManagement();
    TEST_TlsTest_TlsModuleInitialization();
    TEST_TlsTest_SslConfiguration();
    TEST_TlsTest_TlsReadWriteOperations();
    TEST_TlsTest_CertificateInformation();
    TEST_TlsTest_ErrorCodeHandling();
    TEST_TlsTest_BoundaryConditions();
