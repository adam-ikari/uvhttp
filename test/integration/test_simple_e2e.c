...
static int json_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    (void)request;
    const char* json_body = "{\"status\":\"success\",\"message\":\"JSON response\"}";
...
static int send_http_request(const char* method, const char* path, char* response, size_t max_len) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "curl -s -X %s http://127.0.0.1:8770%s 2>/dev/null", method, path);
...
int main(int argc, char** argv) {
    ...
    uvhttp_router_add_route_method(app.router, "/", UVHTTP_DELETE, delete_handler);
