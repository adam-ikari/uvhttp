...
static int validate_json_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
...
static int validate_path_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
...
static int validate_query_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
...
static int validate_headers_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
...
static int send_http_request(const char* method, const char* path, char* response, size_t max_len) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "curl -s -w '\n%%{http_code}' -X %s http://127.0.0.1:8780%s 2>/dev/null", method, path);
...
static int send_http_request_with_body(const char* method, const char* path, const char* body, const char* content_type, char* response, size_t max_len) {
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "curl -s -w '\n%%{http_code}' -X %s -H 'Content-Type: %s' -d '%s' http://127.0.0.1:8780%s 2>/dev/null", method, content_type, body, path);
...
int main(int argc, char** argv) {
    ...
    uvhttp_router_add_route(app.router, "/validate/json", validate_json_handler);
    uvhttp_router_add_route(app.router, "/validate/path", validate_path_handler);
    uvhttp_router_add_route(app.router, "/validate/query", validate_query_handler);
    uvhttp_router_add_route(app.router, "/validate/headers", validate_headers_handler);
    ...
    send_http_request_with_body("POST", "/validate/json", "{\"name\":\"test\"}", "application/json", response, sizeof(response));
    ...
    send_http_request_with_body("POST", "/validate/json", "not json", "application/json", response, sizeof(response));
    ...
    send_http_request_with_body("POST", "/validate/json", "{\"name\":\"test\"}", "text/plain", response, sizeof(response));
    ...
    send_http_request("GET", "/validate/path/../../../etc/passwd", response, sizeof(response));
    ...
    send_http_request("GET", "/validate/query?name=test&age=25", response, sizeof(response));
    ...
    send_http_request("GET", "/validate/query?age=25", response, sizeof(response));
    ...
    send_http_request_with_body("GET", "/validate/headers", "", "application/json", response, sizeof(response));
