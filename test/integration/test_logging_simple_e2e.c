...
static void custom_log_callback(uvhttp_log_level_t level, const char* message, void* user_data) {
    app_context_t* app = (app_context_t*)user_data;
    app->log_count++;
    
    const char* level_str = "UNKNOWN";
    switch (level) {
        case UVHTTP_LOG_TRACE: level_str = "TRACE"; break;
...
static int log_test_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
...
static int log_stats_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    app_context_t* app = (app_context_t*)request->server->context;
...
static int log_level_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    ...
    if (strcmp(level_str, "TRACE") == 0) level = UVHTTP_LOG_TRACE;
    else if (strcmp(level_str, "DEBUG") == 0) level = UVHTTP_LOG_DEBUG;
    else if (strcmp(level_str, "INFO") == 0) level = UVHTTP_LOG_INFO;
    else if (strcmp(level_str, "WARN") == 0) level = UVHTTP_LOG_WARN;
    else if (strcmp(level_str, "ERROR") == 0) level = UVHTTP_LOG_ERROR;
    else if (strcmp(level_str, "FATAL") == 0) level = UVHTTP_LOG_FATAL;
    
    uvhttp_logger_t* logger = uvhttp_logger_new(level, custom_log_callback, request->server->context);
...
static int send_http_request(const char* method, const char* path, char* response, size_t max_len) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "curl -s -X %s http://127.0.0.1:8783%s 2>/dev/null", method, path);
...
static int send_http_request_with_header(const char* method, const char* path, const char* header, char* response, size_t max_len) {
    char cmd[1024];
    snprintf(cmd, sizeof(cmd), "curl -s -X %s -H '%s' http://127.0.0.1:8783%s 2>/dev/null", method, header, path);
...
int main(int argc, char** argv) {
    ...
    send_http_request_with_header("POST", "/log/level", "X-Log-Level: ERROR", response, sizeof(response));
