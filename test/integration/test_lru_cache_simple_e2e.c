...
static int cached_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    ...
    uvhttp_error_t result = uvhttp_lru_cache_get(app->cache, cache_key, (void**)&cached_data, &data_size);
    if (result == UVHTTP_OK) {
        /* Cache hit */
        app->cache_hits++;
    ...
    snprintf(response_data, sizeof(response_data), "Generated response for: %s (cache miss)", cache_key);
...
static int cache_stats_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    app_context_t* app = (app_context_t*)request->server->context;
    ...
    snprintf(stats, sizeof(stats), 
             ...
             (app->cache_hits + app->cache_misses) > 0 ? 
                 (100.0 * app->cache_hits / (app->cache_hits + app->cache_misses)) : 0.0,
...
static int cache_clear_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    app_context_t* app = (app_context_t*)request->server->context;
...
static int send_http_request(const char* method, const char* path, char* response, size_t max_len) {
    char cmd[512];
    snprintf(cmd, sizeof(cmd), "curl -s -X %s http://127.0.0.1:8776%s 2>/dev/null", method, path);
