...
uvhttp_cors_config_t* uvhttp_cors_config_default(void) {
    ...
    config->owns_strings = UVHTTP_TRUE;  /* 标记拥有字符串 */
    ...
    config->allow_all_origins = UVHTTP_TRUE;
    config->allow_credentials_enabled = UVHTTP_FALSE;
...
int uvhttp_cors_middleware(
    ...
    int should_free_config = UVHTTP_FALSE;  /* 标记是否需要释放配置 */
    ...
    if (ctx && ctx->data) {
        ...
        should_free_config = UVHTTP_TRUE;
    }
