...
uvhttp_error_t uvhttp_server_register_protocol_upgrade(
    ...
    if (registry->protocol_count >= 10) {
        UVHTTP_LOG_WARN("Too many protocols registered (max 10), performance may be affected");
        return UVHTTP_ERROR_INVALID_PARAM;
    }
    ...
    if (upgrade_header) {
        strncpy(info->upgrade_header, upgrade_header, sizeof(info->upgrade_header) - 1);
...
uvhttp_error_t uvhttp_connection_transfer_ownership(
    ...
    if (conn->state != UVHTTP_CONN_STATE_HTTP_PROCESSING) {
        UVHTTP_LOG_ERROR("Invalid connection state for ownership transfer: %d", conn->state);
...
uvhttp_error_t uvhttp_connection_get_peer_address(
    ...
    int result = uv_tcp_getpeername(&conn->tcp_handle, (struct sockaddr*)addr, &len);
    if (result != 0) {
