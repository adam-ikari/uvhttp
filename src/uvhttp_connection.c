...
static void on_websocket_read(uv_stream_t* stream, ssize_t nread,
    ...
    if (conn->tls_enabled && conn->ssl && conn->tls_cipher_buf) {
        ...
        for (;;) {
             ...
             * Accumulating up to read_buffer_size (16KB) and stopping
             * leaves plaintext stuck inside mbedtls once the peer has
             * sent everything and no further socket reads arrive — large
             * frames (>16KB) then never complete and the client stalls.
             * The WS parser buffers partial frames across chunks, so
             * per-chunk delivery is safe and frame-size independent. */
            if (ret > 0) {
                result = uvhttp_ws_process_data(
                    ws_conn, (const uint8_t*)conn->read_buffer, (size_t)ret);
                if (result != 0) {
                    UVHTTP_LOG_ERROR("WebSocket TLS data processing failed: %d\n",
                                     result);
                    uvhttp_connection_websocket_close(conn);
                    return;
                }
