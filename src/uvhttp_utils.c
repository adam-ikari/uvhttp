...
static int is_valid_status_code(int code) {
    return (code >= 100 && code <= 599) ? UVHTTP_TRUE : UVHTTP_FALSE;
}

// 内部辅助函数：验证字符串长度
static int is_valid_string_length(const char* str, size_t max_len) {
    if (!str) return UVHTTP_FALSE;
    return (strlen(str) <= max_len) ? UVHTTP_TRUE : UVHTTP_FALSE;
}
 ...
 * @return UVHTTP_TRUE 表示有效，UVHTTP_FALSE 表示无效
 */
int uvhttp_is_valid_status_code(int status_code) {
    return (status_code >= 100 && status_code <= 599) ? UVHTTP_TRUE : UVHTTP_FALSE;
}
