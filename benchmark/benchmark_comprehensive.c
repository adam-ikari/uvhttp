...
/* 全局统计结构（POSIX 允许的例外） */
static comprehensive_stats_t g_stats;
...
static void add_latency_sample(uint64_t latency_us) {
    if (g_g_stats.latency_count < MAX_SAMPLES) {
        g_g_stats.latency_samples[g_g_stats.latency_count++] = latency_us;
        g_g_stats.latency_sum += latency_us;
        
        if (g_g_stats.latency_count == 1) {
            g_g_stats.min_latency = latency_us;
            g_g_stats.max_latency = latency_us;
        } else {
            if (latency_us < g_g_stats.min_latency) g_g_stats.min_latency = latency_us;
            if (latency_us > g_g_stats.max_latency) g_g_stats.max_latency = latency_us;
        }
    }
}
...
static uint64_t calculate_percentile(double percentile) {
    if (g_g_stats.latency_count == 0) return 0;
    
    /* 使用 qsort 进行高效排序 */
    qsort(g_g_stats.latency_samples, g_g_stats.latency_count, sizeof(uint64_t), compare_uint64);
    
    int index = (int)(percentile * g_g_stats.latency_count);
    if (index >= g_g_stats.latency_count) index = g_g_stats.latency_count - 1;
    
    return g_g_stats.latency_samples[index];
}
...
static void print_comprehensive_stats(void) {
    ...
    /* RPS 统计 */
    printf("=== RPS 统计 ===\n");
    printf("总请求数: %d\n", g_g_stats.total_requests);
    printf("成功请求数: %d\n", g_g_stats.successful_requests);
    printf("失败请求数: %d\n", g_g_stats.failed_requests);
    
    if (g_g_stats.total_requests > 0) {
        double success_rate = (double)g_g_stats.successful_requests / g_g_stats.total_requests * 100.0;
        printf("成功率: %.2f%%\n", success_rate);
    }
    
    uint64_t duration_sec = (g_g_stats.end_time - g_g_stats.start_time) / 1000000;
    if (duration_sec > 0) {
        double rps = (double)g_g_stats.successful_requests / duration_sec;
        printf("RPS: %.2f\n", rps);
    }
    printf("\n");
    ...
    /* 延迟统计 */
    printf("=== 延迟统计 ===\n");
    printf("样本数量: %d\n", g_g_stats.latency_count);
    
    if (g_g_stats.latency_count > 0) {
        double avg_latency = g_g_stats.latency_sum / g_g_stats.latency_count;
        ...
        printf("最小延迟: %" PRIu64 " μs (%.3f ms)\n", g_g_stats.min_latency, g_g_stats.min_latency / 1000.0);
        printf("最大延迟: %" PRIu64 " μs (%.3f ms)\n", g_g_stats.max_latency, g_g_stats.max_latency / 1000.0);
    ...
    printf("峰值内存使用: %zu KB (%.2f MB)\n", 
           g_g_stats.peak_memory, g_g_stats.peak_memory / 1024.0);
    printf("当前内存使用: %zu KB (%.2f MB)\n", 
           g_g_stats.current_memory, g_g_stats.current_memory / 1024.0);
    
    if (g_g_stats.successful_requests > 0) {
        double memory_per_request = (double)g_g_stats.peak_memory / g_g_stats.successful_requests;
    ...
    if (duration_sec > 0) {
        ...
        /* 延迟目标: < 15ms */
        if (g_g_stats.latency_count > 0) {
            double avg_latency_ms = g_g_stats.latency_sum / g_g_stats.latency_count / 1000.0;
        ...
        /* 错误率目标: < 0.1% */
        if (g_g_stats.total_requests > 0) {
            double error_rate = (double)g_g_stats.failed_requests / g_g_stats.total_requests * 100.0;
            if (error_rate < 0.1) {
...
static int simple_handler(uvhttp_request_t* request, uvhttp_response_t* response) {
    ...
    if (current_mem > g_g_stats.peak_memory) {
        g_g_stats.peak_memory = current_mem;
    }
    g_g_stats.current_memory = current_mem;
...
static void run_comprehensive_benchmark(const char* test_name) {
    ...
    g_stats.peak_memory = get_memory_usage_kb();
    g_stats.start_time = get_timestamp_us();
    ...
    g_g_stats.end_time = get_timestamp_us();
