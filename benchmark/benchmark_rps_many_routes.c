/**
 * @file benchmark_rps_many_routes.c
 * @brief HTTP 性能基准测试服务器 - 大量路由场景
 * 
 * 这个程序提供标准的 HTTP 服务器用于性能测试，支持 100+ 路由场景：
 * - 测试 Trie 模式下的路由性能
 * - 验证缓存优化对大量路由的影响
...
#define ROUTE_COUNT 50  /* 未超过 HYBRID_THRESHOLD (100) */
...
static void print_usage(const char* program_name, int port) {
    ...
    printf("性能测试示例:\n");
    printf("  # wrk 测试（推荐）\n");
    printf("  wrk -t4 -c100 -d30s http://127.0.0.1:%d/api/v1/users\n", port);
    printf("  wrk -t4 -c100 -d30s http://127.0.0.1:%d/api/v1/products\n", port);
    printf("  wrk -t4 -c100 -d30s http://127.0.0.1:%d/api/v1/orders\n", port);
    printf("\n");
    printf("  # ab 测试\n");
    printf("  ab -n 100000 -c 100 -k http://127.0.0.1:%d/api/v1/users\n", port);
    printf("\n");
    
    printf("不同并发级别测试:\n");
    printf("  wrk -t2 -c10 -d30s  http://127.0.0.1:%d/api/v1/users   (低并发)\n", port);
    printf("  wrk -t4 -c50 -d30s  http://127.0.0.1:%d/api/v1/users   (中等并发)\n", port);
    printf("  wrk -t8 -c200 -d30s http://127.0.0.1:%d/api/v1/users   (高并发)\n", port);
    printf("  wrk -t16 -c500 -d30s http://127.0.0.1:%d/api/v1/users  (极高并发)\n", port);
    printf("\n");
