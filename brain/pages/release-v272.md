...
## 发布时间
2026-09-07，VERSION_TYPE=patch，22 项代码评审缺陷修复补丁版（commit a0eae2b + release commit）

## 包含内容
- **11 项 P0/P1 关键缺陷修复（commit a3bc755）**：query string 路由匹配、MAX_PARAMS 栈溢出边界、on_url/on_header_field 跨 chunk 分段累积、migrate_to_trie 失败悬垂指针、WS 非 TLS send 短写截帧、CLOSE 后继续处理帧（RFC 6455）、If-Modified-Since 时区错误与 3 种日期格式（mktime→timegm，RFC 7231 三种格式）、accept 失败 active_connections 下溢永久 503、connection_new 失败路径 UAF、server_free 不排空在途 close 回调、超时路径 WS wrapper 泄漏
- **11 项 P2/P3 改进与修复（commit a0eae2b）**：TLS EINTR 重试、If-None-Match weak/多值 ETag、目录列表 TOCTOU、on_header_value 分段累积、keep-alive headers_extra 泄漏、X-Forwarded-For 默认不信任（新增 config trust_proxy_headers 开关）、MIME 双表合并单表、TLS cipher 满排空、死代码清理、listen 参数校验、server_stop 幂等化
...
- time: 2026-09-07T03:36:16
  kind: decision
  summary: "v2.7.2 发布：22 项代码评审缺陷修复"
  source: "v2.7.2 发布会话"
  affects: [release-v272]
