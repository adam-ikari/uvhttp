...
class DoxygenXMLParser {
  ...
  generateDocs() {
    ...
    const structsDir = path.join(this.outputDir, 'structs');
    const functionsDir = path.join(this.outputDir, 'functions');
    const definesDir = path.join(this.outputDir, 'defines');
    
    if (!fs.existsSync(structsDir)) fs.mkdirSync(structsDir, { recursive: true });
    if (!fs.existsSync(functionsDir)) fs.mkdirSync(functionsDir, { recursive: true });
    if (!fs.existsSync(definesDir)) fs.mkdirSync(definesDir, { recursive: true });
    ...
    for (const struct of compounds.structs) {
      ...
      try {
        ...
        const outputFile = path.join(structsDir, `${struct.name}.md`);
    ...
    // Convert files (header files to functions, source files to defines)
    ...
    for (const file of compounds.files) {
      ...
      try {
        ...
        if (file.name.endsWith('.h') || file.name.endsWith('.hpp')) {
          outputDir = functionsDir; // Header files to functions
        } else if (file.name.endsWith('.c') || file.name.endsWith('.cpp')) {
          outputDir = definesDir; // Source files to defines
        } else {
          outputDir = this.outputDir; // Other files to root
        }
        
        const outputFile = path.join(outputDir, `${cleanName}.md`);
        fs.writeFileSync(outputFile, md, 'utf-8');
      } catch (error) {
    ...
    // Generate sidebar
    try {
      this.generateSidebar(compounds);
      console.log(`${colors.yellow}Generating sidebar.js...${colors.nc}`);
    } catch (error) {
      console.error(`${colors.red}Error generating sidebar.js: ${error.message}${colors.nc}`);
    }
   ...
   * @param {Object} compounds - Object containing structs, files, and dirs
   * @throws {Error} If writing sidebar.js fails
   */
  generateSidebar(compounds) {
    let sidebarConfig = `// Auto-generated API documentation sidebar configuration
// Generated by scripts/docs/convert_xml_to_markdown.js
export default [
  {
    text: 'API Reference',
    items: [
...
`;
    ...
    if (compounds.structs.length > 0) {
      sidebarConfig += `      {
        ...
        items: [
`;
      const sortedStructs = [...compounds.structs].sort((a, b) => a.name.localeCompare(b.name));
      for (const struct of sortedStructs) {
        sidebarConfig += `          { text: '${struct.name}', link: '/api/generated/structs/${struct.name}' },\n`;
      }
      sidebarConfig += `        ]
      },
...
`;
    ...
    // Add functions section (header files)
    const headerFiles = compounds.files.filter(f => f.name.endsWith('.h') || f.name.endsWith('.hpp'));
    if (headerFiles.length > 0) {
      sidebarConfig += `      {
        ...
        items: [
`;
      const sortedHeaders = [...headerFiles].sort((a, b) => a.name.localeCompare(b.name));
      for (const file of sortedHeaders) {
        const cleanName = file.name.replace(/\//g, '_').replace(/\./g, '_');
        sidebarConfig += `          { text: '${file.name}', link: '/api/generated/functions/${cleanName}' },\n`;
      }
      sidebarConfig += `        ]
      },
...
`;
    ...
    // Add defines section (source files)
    const sourceFiles = compounds.files.filter(f => f.name.endsWith('.c') || f.name.endsWith('.cpp'));
    if (sourceFiles.length > 0) {
      sidebarConfig += `      {
        ...
        items: [
`;
      const sortedSources = [...sourceFiles].sort((a, b) => a.name.localeCompare(b.name));
      for (const file of sortedSources) {
        const cleanName = file.name.replace(/\//g, '_').replace(/\./g, '_');
        sidebarConfig += `          { text: '${file.name}', link: '/api/generated/defines/${cleanName}' },\n`;
      }
      sidebarConfig += `        ]
      },
...
`;
    ...
    const sidebarPath = path.join(this.outputDir, 'sidebar.js');
    try {
      fs.writeFileSync(sidebarPath, sidebarConfig, 'utf-8');
    } catch (error) {
      throw new Error(`Failed to write sidebar.js: ${error.message}`);
    }
  }
}
